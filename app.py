...
def create_app(test_config=None):
    ...
    def guard_request():
        ...
        if request.endpoint not in (
            ...
            if (
                ...
                return jsonify(
                    error="登录已过期，请重新登录。", redirect=url_for("login")
                ), 401
    ...
    def login():
        if request.method == "POST":
            ...
            if hmac.compare_digest(
                user.encode(), app.config["USERNAME"].encode()
            ) and hmac.compare_digest(
                password.encode(), app.config["PASSWORD"].encode()
            ):
                session.clear()
    ...
    def index():
        ...
        if request.args.get("edit"):
            record = parse_record(
                get_db()
                .execute(
                    "SELECT payload FROM rides WHERE date = ?", (request.args["edit"],)
                )
                .fetchone()
            )
            if not record:
        ...
        return render_template(
            "index.html", record=record or {}, configured=bool(config), count=count
        )
    ...
    def settings():
        ...
        return render_template("settings.html", config=dict(row) if row else {})
    ...
    def persist_record(data):
        ...
        css = (Path(app.static_folder) / "story.css").read_text()
    ...
    def upload():
        ...
        target = url_for("generate", date=data["date"])
        if (
            request.path.startswith("/api/")
            or request.headers.get("Accept") == "application/json"
        ):
            return jsonify(
                redirect=target,
                html_url=url_for("cycling_html", filename=filename),
                record=data,
            )
    ...
    @app.get("/generate")
    def generate():
        selected = request.args.get("date") or session.get("last_date")
        if not selected:
            return redirect(url_for("index"))
        record = parse_record(
            get_db()
